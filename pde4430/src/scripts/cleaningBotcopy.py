...
def moveGoal(goal_pose, distance_tolerance):
    ...
    # Then, linear velocity v' = K * sqrt((x'-x)^2 + (y'-y)^2) where K is the constant
    # and sqrt((x'-x)^2 + (y'-y)^2) is the Euclidean distance.
    # The steering angle theta = atan2(y'-y, x'-x) is the angle between these 2 points.
